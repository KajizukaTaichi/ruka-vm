...
|名前|説明|サイズ|読み込み|書き込み|
|---|---|---|---|---|
|プログラム領域|実行されるバイトコードが格納されます|可変長|不可|不可|
|メモリ領域|データやオブジェクトが格納されます|512バイト固定|可能(アドレス指定)|可能(アドレス指定)|
|スタック領域|関数などとデータを受け渡す際に使います。|可変長|可能(ポップのみ)|可能(プッシュのみ)|
|関数スタック|呼び出し元アドレスが格納されます|可変長|不可|不可|
|レジスタユニット|プログラムの実行に必要なレジスタが格納されます|64ビット|可能|可能|
